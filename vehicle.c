...
void init_vehicle(Vehicle *vehicle, Motor left_front_motor, Motor right_front_motor, Motor left_rear_motor, Motor right_rear_motor, Vehicle_PIDs vehicle_pids) {
    vehicle->current_state.odometry_variance.static_error = ENCODER_ERROR **2;
    ...
    vehicle->desired_state.position.heading = 0.0;
    ...
    vehicle->signal_state.position.heading = 0.0;
...
void init_vehicle_pids(Vehicle_PIDs *vehicle_pids , VEL_PID velocity_pid_x ,  VEL_PID velocity_pid_y, VEL_PID velocity_pid_heading , POS_PID pos_pid_x , POS_PID pos_pid_y , POS_PID pos_pid_heading) {
    ...
    vehicle_pids->velocity_pid_heading = velocity_pid_heading;

    vehicle_pids->pos_pid_x = pos_pid_x;
    vehicle_pids->pos_pid_y = pos_pid_y;
    vehicle_pids->pos_pid_heading = pos_pid_heading;
}
...
void translate_twist_to_motor_commands(Vehicle *vehicle) {
    ...
    vehicle->left_front_motor.desired_velocity = (1.0/r) * (vehicle->signal_state.velocity.x - vehicle->signal_state.velocity.y - lx_ly_sum * vehicle->signal_state.velocity.angular);
    vehicle->right_front_motor.desired_velocity = (1.0/r) * (vehicle->signal_state.velocity.x + vehicle->signal_state.velocity.y + lx_ly_sum * vehicle->signal_state.velocity.angular);
    vehicle->left_rear_motor.desired_velocity = (1.0/r) * (vehicle->signal_state.velocity.x + vehicle->signal_state.velocity.y - lx_ly_sum * vehicle->signal_state.velocity.angular);
    vehicle->right_rear_motor.desired_velocity = (1.0/r) * (vehicle->signal_state.velocity.x - vehicle->signal_state.velocity.y + lx_ly_sum * vehicle->signal_state.velocity.angular);
}
